...
public class LeftHeroesBox extends HBox {
    public LeftHeroesBox(Player player) {
        ...
        VBox col2 = new VBox(10); // first column (2 slots)
        col2.setAlignment(Pos.CENTER);

        VBox col3 = new VBox(10); // second column (3 slots)
        col3.setAlignment(Pos.CENTER);

        int count = 0;

        // Add hero cards, starting with the 3-slot column
        for (HeroCard hero : player.getOwnedHero()) {
            if (hero != null && count < 5) {
                CardView card = new CardView(hero, -1);
                if (count < 3) {
                    col3.getChildren().add(card); // fill 3-slot column first
                } else {
                    col2.getChildren().add(card); // then fill 2-slot column
                }
                count++;
            }
        }
        ...
        // Add both columns side by side: 2 slots first, then 3 slots
